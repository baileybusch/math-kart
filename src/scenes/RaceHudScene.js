...
import { openMathStop } from '../ui/mathStop.js';
...
export default class RaceHudScene extends Phaser.Scene {
    ...
    showMathProblem(problem, subtitle, onAnswer, onClose) {
        return openMathStop(this, problem, { subtitle, onAnswer, onClose });
    }
    ...
    showResults(result) {
        ...
        let mathLine = 'Math: ' + result.stats.correct + ' of ' + result.stats.asked + ' right';
        if (result.stats.hints) mathLine += '  \u2022  ' + result.stats.hints + (result.stats.hints === 1 ? ' hint' : ' hints');
